...
async def add_player(client, message):
    ...
    if add_id in all_ids:

        await message.reply_text(
            "<b>⚠️ Player Already Added In League</b>"
        )
    ...
    try:
        ...
        user = await app.get_users(add_id)
...
<b>🏆 League:</b> {active['league']}
"""
    )

    log_text = f"""
<b>🔥 ADMIN DIRECT ADD</b>
